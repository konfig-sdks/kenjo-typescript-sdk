...
import { readableStreamToString, KenjoError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new KenjoError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new KenjoError(e, e.response?.data)
                }
                if (innerError instanceof KenjoError) {
                    // Got "KenjoError" from the above try block
                    throw innerError;
                }
